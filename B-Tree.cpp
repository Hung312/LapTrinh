...
void Scan(NODEPTR p);
void OutputTree(NODEPTR p,int level);	void CreateTree(NODEPTR &Root);
...
int main()
{
	...
	CreateTree(Root);
	OutputTree(Root, 0);
	int X,Position,Found;
	printf ("\nNhap mot so de tien hanh tim kiem: "); scanf("%d",&X);
	if(Search(X,Position,Found))
		printf ("\nTim thay khoa %d o vi tri %d",X,Position);
	else
		printf ("\nKhong tim thay khoa %d",X);
	printf ("\n Nhap mot so de tien hanh xoa: "); scanf("%d",&X);
	DelKey(X);
	OutputTree(Root,0);
	return 0;
}
